...
import { getNewspapersByWeek, type NewspaperData } from "@/lib/newspapers";
...
export default function WeeklyPage() {
  ...
  // Build report from localStorage newspapers
  function buildReportFromStorage(): WeeklyReport | null {
    ...
    // Highlights: top 3 by random selection from all papers
  ...
  const [report, setReport] = useState<WeeklyReport | null>(() => {
    // Try to build from localStorage synchronously on first render
    if (typeof window !== "undefined") {
      try {
        // parseWeekParam and formatDateISO are available in scope
  ...
  // ---- GSAP entrance animation for nav and page chrome ----
  useLayoutEffect(() => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/weekly/generate", {
        ...
        body: JSON.stringify({ week: weekParam, config: getApiConfig() }),
      });
  ...
  return (
    <div className="min-h-screen bg-void relative overflow-hidden pb-20 md:pb-0">
      ...
      <main className="relative z-10 max-w-[900px] mx-auto px-4 sm:px-6 py-10 sm:py-16">
        ...
        {/* ---- Report content or loading/empty state ---- */}
