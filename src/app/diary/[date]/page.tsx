...
export default function DiaryPage() {
  ...
  // ---- Load existing diary on mount / date change ----
  useEffect(() => {
    if (!date) return;
    ...
    async function fetchDiary() {
      ...
      try {
        ...
        // API not available yet – fall back to null
    ...
    // Load newspapers for this date from localStorage
  ...
  const handleGenerate = async () => {
    ...
    // Read user's preferred diary style from settings
  ...
  const handleSaveManual = async () => {
    ...
    setIsSaving(true);
    try {
      // Step 1: Process content via API
      const processRes = await fetch("/api/diary/process", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          content: manualContent,
          processMode,
          style: "newspaper" as DiaryStyle,
          config: getApiConfig(),
        }),
      });

      const processData = await processRes.json();
      const finalContent = processData.processed ?? manualContent;

      // Step 2: Build diary entry
      ...
      setDiary(saveData.diary ?? newDiary);
      setIsManualMode(false);
    } catch {
      // Fallback: save raw content without processing
    ...
    <div className="min-h-screen bg-void relative overflow-hidden pb-20 md:pb-0">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-6 h-[calc(100vh-7rem)]">
          ...
          {/* Right panel – DiaryContent or Manual Editor */}
